...
public class BindingPropertyExtractor implements PropertyExtractor {
    ...
    private void processUnboundedSequence(
        ...
        Collection<Property> props = complexAttr.getProperties();
    ...
    private boolean isUnboundedSequence(Object object, XSDElementDeclaration element) {
        ...
        // if doesn't have unbounded metadata flag, return false
        if (!(complexAttr.getDescriptor().getName() instanceof MetadataSupplier)) return false;
        MetadataSupplier ms = (MetadataSupplier) complexAttr.getDescriptor().getName();
        if (!Boolean.TRUE.equals(ms.getMetadataMap().get("unbounded"))) return false;
        ...
        if (sequenceElement != null) {
            ...
            if ("unbounded".equals(maxOccursString)) return true;
        }
        return false;
    }

    private Element getSequenceElement(XSDElementDeclaration element) {
        final NodeList childNodes = element.getTypeDefinition().getElement().getChildNodes();
